...
**How.** A Python CLI (`subchat.py`) invoked by Igor via Monitor. Reads the per-subagent config from `<SessionFolder>/subchats/<subagent>/config.yaml`, spawns `claude -p` with appropriate flags and cwd, streams progress events to stdout, captures full output to `log/NN/`. Subagent profiles are authored at `instructions/subagents/<name>.md` in the source repo; deploy copies them to `<ContextFolder>/.claude/agents/<name>.md` per-Context — the canonical Claude Code path, so the same files serve both native discovery (`@agent-<name>`, `/agents`) and MCP `spawn_subchat` (which reads them to materialize per-session subchats). Cockpit subagents remain invoked through subchat only; `instructions/igor.md` forbids Task-tool invocation despite native registration.
