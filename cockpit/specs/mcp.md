...
- **Scoping:** **per-context.** The MCP inherits `cwd` from the parent Claude Code process (the ContextFolder root) and operates on `./objectives/`, the current session's `state.md` and `subchats/`, and `./.claude/cockpit/subagents/` (read-only, subagent profiles — explicitly *not* `.claude/agents/`, see `deploy.md`). Historical journal is not touched.
