...
- **Context-local copy:** during `install.py` (see [`cockpit/specs/deploy.md`](../../cockpit/specs/deploy.md)), the source file is copied to `<ContextFolder>/.claude/agents/protocolist.md` — canonical Claude Code path, so Claude Code's native discovery sees it. Deploy overwrites on every install — profile is source-of-truth in the repo. Native registration is for visibility (`@agent-protocolist`, `/agents`); invocation still goes through `subchat`, never the Task tool. The rule lives in [`instructions/igor.md`](../igor.md) §`!протокол`.
- **Runtime materialization:** MCP `spawn_subchat(subagent="protocolist")` reads `<ContextFolder>/.claude/agents/protocolist.md`, splits frontmatter and body, generates `<SessionFolder>/subchats/protocolist/config.yaml` (from frontmatter + defaults per [`cockpit/specs/subchat.md`](../../cockpit/specs/subchat.md)) and `system_prompt.md` (the body).
